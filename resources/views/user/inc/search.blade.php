        ...
        <div class="col-lg-4 col-md-6">
            <div class="form-group">
                ...
                <input id="query" name="query" class="form-control dt" type="text"
                    value="{{ request()->session()->exists('search.form.query')
                        ? request()->session()->get('search.form.query')
                        : '' }}">
            </div>
        </div>
            ...
            <select name="country" class="select-2  form-control" id="country">
                ...
                @foreach (Helper::getList('profile_country') as $profile_country)
                    ...
                    <option data-icon="globe" value="{{ $profile_country->code_ISO }}"
                        {{ request()->session()->get('search.form.country') == $profile_country->code_ISO
                            ? 'selected'
                            : '' }}>
        ...
        <div class="col-lg-3 col-md-6">
            <div class="form-group">
                ...
                <select class="select2-data-ajax form-control   " id="institution" name="institution">
                    <option
                        value="{{ request()->session()->exists('search.form.institution')
                            ? request()->session()->get('search.form.institution')
                            : '' }}"
                        selected="selected">
                        {{ request()->session()->exists('search.form.institution')
                            ? request()->session()->get('search.form.account_name')
                            : '' }}
                    </option>
                </select>
            </div>
        ...
        <div class="col-lg-2 col-md-6">
            <div class="form-group">
                ...
                <select id="order_by" name="order_by" class="form-control">
                    <option value="ASC"
                        {{ request()->session()->get('search.form.order_by') == 'ASC'
                            ? 'selected'
                            : '' }}>
                        ASC
                    </option>
                    <option value="DESC"
                        {{ request()->session()->get('search.form.order_by') == 'DESC'
                            ? 'selected'
                            : '' }}>
                        DESC
                    </option>
