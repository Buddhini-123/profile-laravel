                ...
                <div class="sidebar">
                    <div class="sidebar-content todo-sidebar">
                        <div class="todo-app-menu">
                            <div class="add-task">
                                <a href="/email/create" class="btn btn-primary btn-block">Add Email</a>
                            </div>
                            <div class="sidebar-menu-list">
                                <div class="list-group list-group-filters">
                                    @foreach ($email_type_section as $email_type_section)
                                        <a href="javascript:void(0)" class="list-group-item list-group-item-action">
                                            <i data-feather="mail" class="font-medium-3 mr-50"></i>
                                            <span class="align-middle" id="type"
                                                data-type="{{ $email_type_section->type }}">{{ $email_type_section->type }}</span>
                                        </a>
                                    @endforeach
                                    @foreach ($email_type as $email_type)
                                        <input type="text" id="id" hidden value="{{ $email_type->id }}">
                                    @endforeach
                                </div>
                        ...
                        <div class="todo-app-list">
                            ...
                            <div class="todo-task-list-wrapper list-group">
                                <ul class="todo-task-list media-list email-list" id="todo-task-list">
                                    @foreach ($email_data as $email_data)
                                        <li class="todo-items">
                                                    ...
                                                    <div class="title-wrapper">
                                                        <div class="custom-control custom-checkbox">
                                                            ...
                                                            <label class="click_category custom-control-label"
                                                                for="{{ $email_data->id }}"></label>
                                                        </div>
                                                        <span
                                                            class="click_category todo-title">{{ $email_data->title_eng }}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div class="todo-item-action">
                                                    <div class="badge-wrapper mr-1">
                                                        <a href="/email/edit/{{ $email_data->id }}"
                                                            class="btn btn-flat-warning waves-effect">Edit </a>
                                                        <small class="text-nowrap text-muted mr-1">{{ $email_data->date_created }}</small>
                                                    </div>
                                                </div>
                                            </div>
                                        </li>
